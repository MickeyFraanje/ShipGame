...
//Sets the direction for the ship in a way that allows multiple ships.
  ...
  //  shipHit.play();
    ...
    health--; //Health is detracted.
    if(purpleScore - 20 > 0 && health > 1){ //If 40 points can be detracted without going below 0, 40 points are detracted.
      purpleScore -= 20;
    }else if(purpleScore - 20 <= 0 && health > 1){
      purpleScore = 0; //If score - 40 is < 0, new score = 0;
    }else{
    }
