...
void setup() {
  ...
  onePlayer = new Button(width/2, (height/2)-75, 350, 100, "Start"); //Button to start the game in singleplayer mode.
  highScoreButton = new Button(width/2, (height/2)+75, 350, 100, "High Score"); //Used to go to the high score menu.
...
void updateHighScore() {
  ...
  if (highScore[9] <= purpleScore ) { //Checks if the total score is higher than the lowest on the list.
    highScore[9] = purpleScore; //If it is higher, it replaces it.
...
void nuggetHit() {
  ...
  while (nuggetIter.hasNext()) { //Detects if the purple ship picks the nugget.
